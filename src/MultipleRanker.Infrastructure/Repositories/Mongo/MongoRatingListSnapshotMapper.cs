            ...
            var config = new MapperConfiguration(cfg =>
            {
                ...
                cfg.CreateMap<RatingListParticipantSnapshot, RatingListParticipantSnapshotEntity>()
                    ...
                    .ForMember(x => x.TotalScoreByOpponentId, opt
                        => opt.MapFrom(x => x.TotalScoreByOpponentId.Select(y => new ValueByOpponentIdEntity<double> { OpponentId = y.Key.ToString(), Value = y.Value })))
                    .ForMember(x => x.TotalScoreConcededByOpponentId, opt
                        => opt.MapFrom(x => x.TotalScoreConcededByOpponentId.Select(y => new ValueByOpponentIdEntity<double> { OpponentId = y.Key.ToString(), Value = y.Value })))
                    .ForMember(x => x.TotalWinsByOpponentId, opt
                        => opt.MapFrom(x => x.TotalWinsByOpponentId.Select(y => new ValueByOpponentIdEntity<int> { OpponentId = y.Key.ToString(), Value = y.Value })));
